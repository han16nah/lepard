...
void brute_neighbors(vector<PointXYZ>& queries, vector<PointXYZ>& supports, vector<int>& neighbors_indices, float radius, int verbose)
{
	...
	for (auto& p0 : queries)
	{
		...
		for (auto& p : supports)
		{
			...
			{
				tmp[i0].push_back(i);
				if (static_cast<int>(tmp[i0].size()) > max_count)
	...
	for (auto& inds : tmp)
	{
		...
		{
			if (static_cast<size_t>(j) < inds.size())
	...
	for (auto& p0 : queries)
	{
		...
		for (auto& p : supports)
		{
			...
			if (d2 < r2)
			{
			    ...
			    // Update max count
				if (static_cast<int>(tmp[i0].size()) > max_count)
					max_count = static_cast<int>(tmp[i0].size());
			}
			i++;
		}
		i0++;
                                ...
                                float radius)
{
	...
	for (auto& p0 : queries)
	{
        ...
        // Update max count
        if (nMatches > static_cast<size_t>(max_count))
            max_count = static_cast<int>(nMatches);
	...
	for (auto& inds_dists : all_inds_dists)
	{
		...
		for (int j = 0; j < max_count; j++)
		{
			if (static_cast<size_t>(j) < inds_dists.size())
