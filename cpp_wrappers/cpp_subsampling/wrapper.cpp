...
static const char module_docstring[] = "This module provides an interface for the subsampling of a batch of stacked pointclouds";

static const char subsample_docstring[] = "function subsampling a pointcloud";

static const char subsample_batch_docstring[] = "function subsampling a batch of stacked pointclouds";
...
static PyObject* batch_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|$OOfsii", const_cast<char**>(kwlist), &points_obj, &batches_obj, &features_obj, &classes_obj, &sampleDl, &method_buffer, &max_p, &verbose))
	{
...
static PyObject* cloud_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|$OOfsi", const_cast<char**>(kwlist), &points_obj, &features_obj, &classes_obj, &sampleDl, &method_buffer, &verbose))
	{
		PyErr_SetString(PyExc_RuntimeError, "Error parsing arguments");
		return NULL;
	}
